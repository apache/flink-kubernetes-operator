...
Everything the operator knows about a snapshot is reported through the resource status, whose state machine is described under [Custom Resource → Overview]({{< ref "docs/custom-resource/overview#flinkstatesnapshot" >}}) and whose fields are listed in the [Reference]({{< ref "docs/custom-resource/reference" >}}), and mirrored into the labels described [above](#advanced-snapshot-filtering). How the status is persisted, and what deleting a resource erases, is covered under [State → Custom Resource Status]({{< ref "docs/operations/state#custom-resource-status" >}}).
