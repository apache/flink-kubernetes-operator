...
The operator is extensible through plugins: separately compiled implementations that are mounted at deployment time and discovered when the operator starts. They fall into two families, plugins that hook into how the custom resources are validated, mutated, and observed, and plugins that extend the phases of the autoscaler loop, both packaged into the operator image, or onto the standalone autoscaler's classpath.

## Additional Dependencies
Some setups need additional dependencies on the operator classpath. When building a custom image, they are copied to `/opt/flink/operator-lib`, exposed as the `OPERATOR_LIB` environment variable, and that folder is added to the classpath upon initialization:
...
Plugin implementations follow a different path: they are loaded through the Flink [Plugins](https://nightlies.apache.org/flink/flink-docs-master/docs/deployment/filesystems/plugins) mechanism from the directory set by `FLINK_PLUGINS_DIR` (`/opt/flink/plugins` in the operator image), each plugin in its own subdirectory, as shown step by step in the sections below. The same directory also hosts Flink filesystem plugins: fetching `FlinkSessionJob` artifacts from schemes such as `s3` or `hdfs` goes through Flink's FileSystem API, so the matching filesystem plugin must be added there, with the scheme allowed as described under [Security]({{< ref "docs/deployment/security#artifact-fetching" >}}).

## Resource Plugins

The operator's handling of the custom resources is extensible at three points: validation before a spec is accepted, mutation as resources are admitted, and listeners observing every resource event.

### Custom Flink Resource Validators

`FlinkResourceValidator`, an interface for validating the resources of `FlinkDeployment`, `FlinkSessionJob` and `FlinkStateSnapshot`, is a pluggable component based on the [Plugins](https://nightlies.apache.org/flink/flink-docs-master/docs/deployment/filesystems/plugins) mechanism. A custom implementation of `FlinkResourceValidator` is packaged together with its service definition in `META-INF/services`.
Validators run in two places: the validating admission webhook rejects an invalid spec already at apply time, and the controllers validate again during reconciliation, so an invalid spec is caught even when the webhooks are disabled, with the error reported on the resource status.
The following steps demonstrate how to develop and use a custom validator.

1. Implement `FlinkResourceValidator` interface:
    ```java
    package org.apache.flink.validator;
    ...
    public class CustomValidator implements FlinkResourceValidator {
        ...
        public Optional<String> validateStateSnapshot(
                FlinkStateSnapshot stateSnapshot, Optional<AbstractFlinkResource<?, ?>> target) {
            return Optional.empty();
        }
    }
    ```

2. Create service definition file `org.apache.flink.kubernetes.operator.api.validation.FlinkResourceValidator` in `META-INF/services`. With a custom `FlinkResourceValidator` implementation, the service definition is as follows:
    ```text
    org.apache.flink.validator.CustomValidator
    ```

3. Use the Maven tool to package the project and generate the custom validator JAR.

4. Create a Dockerfile to build a custom image from the `apache/flink-kubernetes-operator` official image and copy the generated JAR to custom validator plugin directory.
...
5. Install the flink-kubernetes-operator helm chart with the custom image and verify the `deploy/flink-kubernetes-operator` log has:
    ```text
    o.a.f.k.o.u.ValidatorUtils [INFO ] Discovered resource validator from plugin directory[/opt/flink/plugins]: org.apache.flink.validator.CustomValidator.
    ```

### Custom Flink Resource Mutators

`FlinkResourceMutator`, an interface for mutating the resources of `FlinkDeployment`, `FlinkSessionJob` and `FlinkStateSnapshot`, is a pluggable component based on the [Plugins](https://nightlies.apache.org/flink/flink-docs-master/docs/deployment/filesystems/plugins) mechanism. A custom implementation of `FlinkResourceMutator` is packaged together with its service definition in `META-INF/services`.
Mutators run only in the mutating admission webhook: when the webhooks are disabled (`webhook.create: false`, or `webhook.mutator.create: false` for the mutating webhook alone), custom mutators are not applied.
...
1. Implement `FlinkResourceMutator` interface:
   ...
   /** Custom mutator implementation of {@link FlinkResourceMutator}. */
...
2. Create service definition file `org.apache.flink.kubernetes.operator.api.mutator.FlinkResourceMutator` in `META-INF/services`. With a custom `FlinkResourceMutator` implementation, the service definition is as follows:
    ```text
...
4. Create a Dockerfile to build a custom image from the `apache/flink-kubernetes-operator` official image and copy the generated JAR to custom mutator plugin directory.
...
5. Install the flink-kubernetes-operator helm chart with the custom image and verify the `deploy/flink-kubernetes-operator` log has:
    ```text
    o.a.f.k.o.u.MutatorUtils [INFO ] Discovered mutator from plugin directory[/opt/flink/plugins]: org.apache.flink.mutator.CustomFlinkMutator.
    ```

### Custom Flink Resource Listeners

`FlinkResourceListener`, an interface for listening to the events and status updates of `FlinkDeployment`, `FlinkSessionJob` and `FlinkStateSnapshot` resources, is a pluggable component based on the [Plugins](https://nightlies.apache.org/flink/flink-docs-master/docs/deployment/filesystems/plugins) mechanism. The listener methods are called after the respective events have been triggered by the system, and the context passed to each method exposes the resource, the timestamp, and the `KubernetesClient` itself, enabling tighter integration with external data platforms. A custom implementation of `FlinkResourceListener` is packaged together with its service definition in `META-INF/services`.
The following steps demonstrate how to develop and use a custom listener.

1. Implement `FlinkResourceListener` interface:
    ```java
    package org.apache.flink.listener;
    ...
    /** Custom listener implementation of {@link FlinkResourceListener}. */
    public class CustomFlinkResourceListener implements FlinkResourceListener {

        @Override
        public void onDeploymentStatusUpdate(
                StatusUpdateContext<FlinkDeployment, FlinkDeploymentStatus> ctx) {
            // react to the new deployment status, for example forward it to an external system
        }

        @Override
        public void onDeploymentEvent(ResourceEventContext<FlinkDeployment> ctx) {
            // react to the Kubernetes event of the deployment
        }

        @Override
        public void onSessionJobStatusUpdate(
                StatusUpdateContext<FlinkSessionJob, FlinkSessionJobStatus> ctx) {
            // react to the new session job status
        }

        @Override
        public void onSessionJobEvent(ResourceEventContext<FlinkSessionJob> ctx) {
            // react to the Kubernetes event of the session job
        }

        @Override
        public void onStateSnapshotEvent(FlinkStateSnapshotEventContext ctx) {
            // react to the snapshot event
        }

        @Override
        public void onStateSnapshotStatusUpdate(FlinkStateSnapshotStatusUpdateContext ctx) {
            // react to the new snapshot status
        }
    }
    ```

2. Create service definition file `org.apache.flink.kubernetes.operator.api.listener.FlinkResourceListener` in `META-INF/services`. With a custom `FlinkResourceListener` implementation, the service definition is as follows:
    ```text
    org.apache.flink.listener.CustomFlinkResourceListener
    ```

3. Use the Maven tool to package the project and generate the custom listener JAR.

4. Create a Dockerfile to build a custom image from the `apache/flink-kubernetes-operator` official image and copy the generated JAR to custom listener plugin directory.
    `/opt/flink/plugins` is the value of `FLINK_PLUGINS_DIR` environment variable in the flink-kubernetes-operator helm chart. The structure of custom listener directory under `/opt/flink/plugins` is as follows:
    ```text
    /opt/flink/plugins
        ├── custom-listener
        │   ├── custom-listener.jar
    ...
    With the custom listener directory location, the Dockerfile is defined as follows:
    ```shell script
    FROM apache/flink-kubernetes-operator
    ENV FLINK_PLUGINS_DIR=/opt/flink/plugins
    ENV CUSTOM_LISTENER_DIR=custom-listener
    RUN mkdir $FLINK_PLUGINS_DIR/$CUSTOM_LISTENER_DIR
    COPY custom-listener.jar $FLINK_PLUGINS_DIR/$CUSTOM_LISTENER_DIR/
    ```

5. Enable the listener in the [operator configuration]({{< ref "docs/deployment/configuration" >}}). A discovered listener is activated only when it is explicitly named under `kubernetes.operator.plugins.listeners.<name>.class`, and any further keys under the same prefix are passed to the listener through its `configure()` method:
    ```yaml
    kubernetes.operator.plugins.listeners.custom-listener.class: org.apache.flink.listener.CustomFlinkResourceListener
    kubernetes.operator.plugins.listeners.custom-listener.k1: v1
    ```
    A listener that is discovered but not configured is skipped with a "`No configuration found`" warning.

6. Install the flink-kubernetes-operator helm chart with the custom image and verify the `deploy/flink-kubernetes-operator` log has:
    ```text
    o.a.f.k.o.l.ListenerUtils [INFO ] Discovered resource listener from plugin directory[/opt/flink/plugins]: org.apache.flink.listener.CustomFlinkResourceListener.
    ```

## Autoscaler Plugins

The autoscaler exposes one extension seam per phase of its loop: custom evaluators shape the evaluated metrics, custom alignment modes decide which parallelisms are acceptable, and custom executors get the final word on a scaling decision. Where each seam sits in the loop is described under [Autoscaler]({{< ref "docs/managing/autoscaler" >}}).
...
2. Create the service definition file `org.apache.flink.autoscaler.metrics.ScalingMetricsEvaluatorPlugin` in `META-INF/services` with the fully-qualified class name of the implementation:
    ```text
...
1. Implement `org.apache.flink.autoscaler.alignment.ParallelismAlignmentMode`:
    ...
    /** Custom alignment mode implementation of {@link ParallelismAlignmentMode}. */
    public class CustomAlignmentMode implements ParallelismAlignmentMode {
        ...
        /** Snap down to the nearest parallelism that divides the key groups or partitions evenly. */
        @Override
...
2. Create the service definition file `org.apache.flink.autoscaler.alignment.ParallelismAlignmentMode` in `META-INF/services`:
    ```text
    org.apache.flink.autoscaler.custom.CustomAlignmentMode
    ```

3. Use the Maven tool to package the project and generate the custom alignment mode JAR.

4. Select the custom mode by name and point it at the implementation class. Any other `job.autoscaler.scaling.parallelism-alignment.mode.<name>.*` entries are merged (prefix-stripped) on top of the job configuration and exposed to the mode through `Context#getConfiguration()`:
    ```yaml
    job.autoscaler.scaling.parallelism-alignment.mode: custom-mode
    job.autoscaler.scaling.parallelism-alignment.mode.custom-mode.class: org.apache.flink.autoscaler.custom.CustomAlignmentMode
    job.autoscaler.scaling.parallelism-alignment.mode.custom-mode.min-parallelism: 4
    ```
   {{< hint info >}}
   The `<name>` is a free-form identifier. `custom-mode` is used here, but `CUSTOM_MODE` or any other style works just as well. It is matched exactly, including case, so the value of `scaling.parallelism-alignment.mode` and the `<name>` in `scaling.parallelism-alignment.mode.<name>.class` must be identical. As a result, `CUSTOM_MODE` and `custom-mode` are two different modes, not aliases. The only reserved names are the built-ins (`BALANCED`, `EVENLY_SPREAD`, `OFF`), which always resolve to the built-in modes regardless of any configured class.
   {{< /hint >}}
...
5. Deploy the mode.
    ...
    - **Operator deployment** - create a Dockerfile to build a custom image from the `apache/flink-kubernetes-operator` official image and copy the generated JAR to a custom alignment mode plugin directory under `/opt/flink/plugins` (the value of the `FLINK_PLUGINS_DIR` environment variable in the flink-kubernetes-operator helm chart). The structure of the custom alignment mode directory under `/opt/flink/plugins` is as follows:
        ...
        Install the flink-kubernetes-operator helm chart with the custom image and verify the `deploy/flink-kubernetes-operator` log has:
        ```text
        o.a.f.k.o.u.AutoscalerUtils [INFO ] Discovered custom alignment mode for autoscaler from plugin directory[/opt/flink/plugins]: org.apache.flink.autoscaler.custom.CustomAlignmentMode.
        ```
    ...
    - **Standalone autoscaler** - simply place the custom alignment mode JAR on the classpath of the `flink-autoscaler-standalone` process. It will be picked up automatically via Java's `ServiceLoader` and discovery will be logged:
        ```text
        o.a.f.a.s.u.AutoscalerUtils [INFO ] Discovered custom alignment mode via ServiceLoader: org.apache.flink.autoscaler.custom.CustomAlignmentMode.
        ```

### Custom Scaling Executors

`ScalingExecutorPlugin` is a pluggable component for intercepting, modifying, or vetoing computed scaling decisions before they are applied. Custom scaling executors are discovered through the [Plugins](https://nightlies.apache.org/flink/flink-docs-master/docs/deployment/filesystems/plugins) mechanism when running inside the Kubernetes operator, and through the standard Java `ServiceLoader` mechanism when running with `flink-autoscaler-standalone`. In both cases the implementation class must be registered in `META-INF/services`.
...
1. Implement the `ScalingExecutorPlugin` interface:
    ...
    /** Custom scaling executor implementation of {@link ScalingExecutorPlugin}. */
...
2. Create the service definition file `org.apache.flink.autoscaler.ScalingExecutorPlugin` in `META-INF/services` with the fully-qualified class name of the implementation:
    ```text
