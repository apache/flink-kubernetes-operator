...
Apache Flink runs natively on Kubernetes: it can bring up a JobManager and TaskManagers and submit a job. But a streaming application lives for months, and that native integration effectively stops at deployment, everything that comes afterward, restarting unhealthy jobs, savepointing before upgrades, rolling back bad releases, resizing as load shifts, stays with a person who understands Flink, indefinitely.

The Flink Kubernetes Operator takes that work off human hands, following the Kubernetes [operator pattern](https://kubernetes.io/docs/concepts/extend-kubernetes/operator/): the desired application is declared as a [custom resource](https://kubernetes.io/docs/concepts/extend-kubernetes/api-extension/custom-resources/), and the operator continuously reconciles the running cluster toward it, observing what runs, comparing it against the declaration, and acting to close any gap. Flink applications become first-class Kubernetes objects, managed and secured with the same tooling as any other workload.

The automation is only part of it. Across the full load range, from saturation to idle, the operator optimizes for both performance and cost: under load, pipelines hold their throughput targets and upgrades roll out with no dip, and when demand falls, parallelism and cluster resources are trimmed, so nothing keeps paying for idle capacity.

<!-- The exported SVG embeds its own editable draw.io source: open it directly in draw.io to modify the figure. -->
{{< img src="/img/concepts/overview-deliverables.svg" alt="The operator deliverables at a glance" >}}

Three properties round it out:

- Scale: a single lightweight instance, on a modest resource footprint, is built to manage hundreds or thousands of pipelines at once.
- Coverage: long-running streaming jobs and bounded batch jobs alike, on Kubernetes in [native or standalone mode]({{< ref "docs/deployment/overview#operator-deployment-modes" >}}).
- Currency: the operator integrates with the latest Flink versions and their features as they arrive, listed under [Compatibility → Supported Flink Versions]({{< ref "docs/deployment/compatibility#supported-flink-versions" >}}).

Reducing human intervention to a minimum does not remove the need to understand the operator: knowing what it can do, how it manages a job's lifecycle, and where its limits lie is what makes it safe to trust with production workloads, and the rest of this documentation builds that understanding.
...
- **[Lifecycle Management]({{< ref "docs/concepts/lifecycle-management" >}})**: the operator runs the full life of an application, deploying it, keeping it running, and cleanly suspending or deleting it. It upgrades jobs statefully or statelessly, snapshots their state declaratively, rolls back failed upgrades, and restarts unhealthy jobs on its own, so routine operations and common failures no longer need a person on call.
- **[Zero-Downtime Upgrades]({{< ref "docs/concepts/zero-downtime-upgrades" >}})**: for jobs that cannot tolerate the brief restart of a normal upgrade, blue/green deployments bring the new version up alongside the old one and switch over only once it is proven healthy, aborting back to the old version when it is not, so an upgrade never interrupts the stream.
- **[Autoscaling]({{< ref "docs/concepts/autoscaling" >}})**: the operator measures each job's real workload and continuously right-sizes its parallelism, adding capacity under load and releasing it as demand falls, with TaskManager memory tuned alongside, keeping jobs on their throughput targets without over-provisioning.
- **Kubernetes-Native Operations**: everything an application needs to run well in a cluster is treated as a first-class concern, from Helm installation, RBAC, and high availability to metrics, logging, events, and ingress, so the operator fits existing Kubernetes practices instead of standing apart from them.
...
- **Official Images**: published to [GHCR](https://github.com/apache/flink-kubernetes-operator/pkgs/container/flink-kubernetes-operator) and [DockerHub](https://hub.docker.com/r/apache/flink-kubernetes-operator).
