...
The whole system revolves around managing the Flink custom resources:

- Users define their desired state in the resource spec, and the operator works on achieving it, updating the status along the way.
- The Flink clusters are managed exclusively by the operator, each being the realization of the spec defined by the user.

The custom resources themselves, from their overall structure down to every configurable field, are covered under [Custom Resource → Overview]({{< ref "docs/custom-resource/overview" >}}), and deploying the operator and the webhook under [Deployment → Overview]({{< ref "docs/deployment/overview" >}}).
...
Every admission request is delivered by the API server to the webhook's TLS endpoint, whose certificate rotates without a restart. Once received, the request passes through two stages:
