...
{{< hint info >}}
The stabilization samples are still collected for a reason. Right after a start or rescale the job is typically catching up, which is exactly when the observed processing rate of a source is measurable, and those early observations seed the backpressure-based capacity estimate used once the window starts filling, as described under [Busy Time and Observed Rates](#busy-time-and-observed-rates).
{{< /hint >}}

#### Window Filling

The samples that follow are averaged over the sliding window set by `job.autoscaler.metrics.window` (default 15 minutes). The first evaluation happens once the window is full, and from then on every pass re-evaluates the window as it slides forward by one sample, until a decision triggers a rescale and the cycle starts over.

The window size is the main smoothing knob: a larger window makes decisions calmer but slower to react, a smaller one reacts faster but is more exposed to fluctuations. Values between 3 and 60 minutes work well in practice, depending on how spiky the load is.
...
The stored history follows three rules:

- Samples survive operator restarts: every pass persists them in the autoscaler ConfigMap under the `collectedMetrics` entry, stabilization included, settling samples are stored like any other and only never count toward the window.
- Once the window is full, every pass trims the stored history down to the window.
- When the job restarts, the history is cleared entirely and collection starts from scratch.
...
For sources there is a direct measurement to fall back on, driven by backpressure time instead of busy time: while a source works through a real backlog, its ingest rate divided by the share of time it is not backpressured is the capacity it demonstrably delivers, the observed true processing rate. Three options gate the observation and the switch:

| Key                                                             | Default    | Role                                                                                                 |
|-----------------------------------------------------------------|------------|------------------------------------------------------------------------------------------------------|
| `job.autoscaler.observed-true-processing-rate.lag-threshold`    | 30 seconds | Samples are taken only while the source lags behind by at least this much                            |
| `job.autoscaler.observed-true-processing-rate.min-observations` | `2`        | Samples needed before the observed rate is trusted                                                   |
| `job.autoscaler.observed-true-processing-rate.switch-threshold` | `0.15`     | Switch to the observed rate when the busy-time estimate is at least this much higher during catch-up |

Whichever estimate prevails is the vertex's true processing rate, its current capacity, the number the required capacity is later checked against and divided by:
...
The whole model rests on the input rate, and that number is less obvious than it looks: it is not simply what a vertex currently reads. The evaluation walks the pipeline from the sources downstream, deriving every rate along the way:

- A source is credited with its ingestion rate, what it reads plus the rate at which its lag grows, so a source falling behind counts the load that actually arrives at the external system, not just the share it keeps up with.
- Every edge carries an output ratio, how many records the upstream vertex sends down that edge for every record it receives, and multiplying the upstream's target rate by the ratio gives the data rate of the edge.
- A vertex's own data rate is the sum across its incoming edges, so no vertex below the sources gets its rate from its own measurements.

The catch-up rate for the backlog travels the same way:
...
In order to provide stable job performance and some buffer for load fluctuations, the autoscaler works toward a configurable target utilization, with the upper and lower thresholds bounding the band within which no scaling action is taken:

| Key                                 | Default        | Role                                                                                       |
|-------------------------------------|----------------|--------------------------------------------------------------------------------------------|
| `job.autoscaler.utilization.target` | `0.7`          | The utilization the required capacity is sized for, `0.6` meaning 60% for the job vertices |
| `job.autoscaler.utilization.max`    | `target + 0.3` | Utilization above this triggers a scale-up                                                 |
| `job.autoscaler.utilization.min`    | `target - 0.3` | Utilization below this triggers a scale-down                                               |

It is not recommended to set the target close to the extremes: near 100%, performance usually degrades once capacity limits are reached in most real-world systems, while near 0% the job is heavily over-provisioned, reserving capacity that sits idle and defeating the cost benefits of scaling.
...
Rescaling itself costs time: while the job restarts, backlog accumulates, and the new deployment must process it on top of the incoming load. The autoscaler reserves extra capacity for exactly that, sized from the expected restart time, detailed in the next subsection, and from two options:

| Key                                               | Default    | Role                                                                                                                                                                                                                                                |
|---------------------------------------------------|------------|-----------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------|
| `job.autoscaler.catch-up.duration`                | 30 minutes | The time within which accumulated backlog should be fully processed after a rescale. Lowering it reserves more spare capacity, set it to the delivery objective, for example 10, 30 or 60 minutes, or `0` to disable backlog-based scaling entirely |
| `job.autoscaler.backlog-processing.lag-threshold` | 5 minutes  | Lag at any source worth more than this much processing time counts as an actual backlog                                                                                                                                                             |

The backlog term acts in both directions:

- It pulls scale-up forward: the required capacity rises, so a backlogged vertex is scaled earlier and further, and the shorter the catch-up window, the more aggressive the step.
- It holds scale-down back: while an actual backlog is being drained, scale-down is suspended entirely, avoiding decisions that would create more lag.
...
| Key                                            | Default    | Role                                                                           |
|------------------------------------------------|------------|--------------------------------------------------------------------------------|
| `job.autoscaler.restart.time`                  | 5 minutes  | The static estimate, in effect until tracking has observed a rescale           |
| `job.autoscaler.restart.time-tracking.enabled` | `false`    | Use the maximum restart duration observed across the recorded rescales instead |
| `job.autoscaler.restart.time-tracking.limit`   | 15 minutes | Upper cap on the tracked estimate                                              |

Tracking is worth enabling once a job has been through a few rescales: it replaces a guess with the job's own worst observed case. The estimate matters in both directions: an underestimated restart leaves the job lagging longer than the catch-up window promises, while an overestimated one reserves capacity that is never needed.
...
Execution starts where evaluation ended, at the required capacity, and turns it into the parallelism it applies:

1. Dividing the required capacity by the current capacity, the measured true processing rate, gives the scale factor.
2. The observed scalability coefficient reshapes the factor when learning from history is enabled, and the maximum scale-up and scale-down factors cap it.
3. The factor is multiplied onto the current parallelism, and the product is clamped to the configured bounds.
4. For keyBy vertices and partitioned sources, alignment snaps the result to the key groups or partitions, yielding the recommended parallelism.
5. The recommendation clears the blocking barrier of the scaling execution guards, the custom executors shape it one final time, and only what survives is written to the job as its new parallelism:
...
By default the scale factor assumes linear scaling: doubling the parallelism doubles the capacity. Real pipelines rarely scale perfectly, coordination and network overhead eat part of every step, and learning can correct for it: a scalability coefficient is fitted over the vertex's own scaling history, the true processing rates observed at each past parallelism, and the scale factor is corrected by it:

| Key                                                    | Default  | Role                                                                                                                                              |
|--------------------------------------------------------|----------|---------------------------------------------------------------------------------------------------------------------------------------------------|
| `job.autoscaler.observed-scalability.enabled`          | `false`  | Learn the coefficient from history instead of assuming linear scaling                                                                             |
| `job.autoscaler.observed-scalability.min-observations` | `3`      | Past decisions needed for the fit, linear scaling assumed until then                                                                              |
| `job.autoscaler.observed-scalability.coefficient-min`  | `0.5`    | Lower clamp of the coefficient, capped at `1.0` above, so a low estimate cannot cause runaway scale-ups and super-linear scaling is never assumed |
| `job.autoscaler.history.max.count`                     | `3`      | Scaling history entries retained per vertex                                                                                                       |
| `job.autoscaler.history.max.age`                       | 24 hours | Age limit of the retained history                                                                                                                 |

The history itself, one entry per executed scaling, is persisted per vertex under the `scalingHistory` entry of the autoscaler ConfigMap. Raising `min-observations` therefore only works together with a matching `history.max.count`, and both are bounded by what the state store can hold. The coefficient is not the history's only consumer: the effectiveness detection below judges the latest history entry, and the scaling tracking matches its restart durations against the recorded targets.

#### Parallelism Bounds

The computed parallelism is bounded on the absolute axis and per step:

| Key                                     | Default    | Role                                                                                              |
|-----------------------------------------|------------|---------------------------------------------------------------------------------------------------|
| `job.autoscaler.vertex.min-parallelism` | `1`        | Lower bound of any vertex parallelism                                                             |
| `job.autoscaler.vertex.max-parallelism` | `200`      | Upper bound, never exceeding the max parallelism configured for the vertex itself                 |
| `job.autoscaler.scale-down.max-factor`  | `0.6`      | At most a 60% reduction in one step, the new parallelism never drops below 40% of the current one |
| `job.autoscaler.scale-up.max-factor`    | `100000.0` | Cap on one step's growth, effectively unbounded by default                                        |

For sources that report a partition count, Kafka and Pulsar do by default, the partition count acts as an additional upper bound, since scaling a source beyond its partitions would only add idle subtasks.
...
In the figure, `N` is the number a vertex aligns to, its key groups for a keyBy vertex or the partition count for a source, and `p` is the candidate parallelism the search tests:

- `EVENLY_SPREAD` accepts `p` only when it divides `N` without remainder.
- `BALANCED` also accepts the first `p` that lowers the biggest per-subtask share relative to the computed target, which is what `N / p < N / target` expresses in whole key groups or partitions.
- The upper align limit is the smallest of `N`, the vertex's max parallelism, and `job.autoscaler.vertex.max-parallelism`, so alignment never proposes more subtasks than there is data to distribute.
- The lower align limit is the `job.autoscaler.vertex.min-parallelism` floor, which the computed target already respects through the [Parallelism Bounds](#parallelism-bounds) above.
...
Because both searches move upward, the built-in modes carry a structural bias toward more parallelism: a scale-up may be aligned further than the computed target, while a scale-down is softened back toward the current parallelism, aggressive up, conservative down.

Custom alignment modes are the answer whenever this does not fit: they can implement a different trade-off, treat the parallelism ranges in any other way, or extend the alignment to vertices the built-ins do not recognize. Between vertices, only keyBy (hash) partitioning is handled by default, so a pipeline that partitions data with a custom scheme can bring its own mode.
...
- Cluster capacity: scale-ups can be validated against what the cluster can actually hold, enabled by setting `kubernetes.operator.cluster.resource-view.refresh-interval` to a positive duration (disabled by default):
  - The operator periodically reads the allocatable CPU and memory of the cluster nodes and simulates placing the TaskManagers the rescaled job would need, estimated from the new parallelisms and `taskmanager.numberOfTaskSlots`. An action that would not fit is blocked, which above all prevents mass scale-ups after an outage from exhausting the cluster.
  - The simulation is aware of the [Kubernetes Cluster Autoscaler](https://github.com/kubernetes/autoscaler/tree/master/cluster-autoscaler): when one is detected, the node view is extrapolated up to the maximum size of each autoscaled node group, so capacity it can still provision counts as available.
  - Independently of the view, an action is also held back while the JobManager has not reported its task slot usage yet, typically right after the job comes up.
...
The scale-down interval does more than postpone the action:

- The first scale-down recommendation for a vertex only opens the window, nothing is applied yet, and every following pass records what it would have scaled the vertex down to.
- Once recommendations have kept pointing down for the whole interval, the scale-down executes with the highest parallelism recommended within the window rather than the latest one, so a brief deep dip cannot drag the vertex to its lowest recorded target.
- A pass recommending the current parallelism or more discards the pending request, and any executed rescale clears all pending requests as well.
- The requests are persisted in the autoscaler ConfigMap under the `delayedScaleDown` entry, so an operator restart does not reset a window that is already running.
...
- The cluster capacity check extrapolates autoscaling headroom only for the Kubernetes Cluster Autoscaler on EKS: the detection joins the autoscaler's status ConfigMap against the `eks.amazonaws.com/nodegroup` node label, so on other clusters, and for provisioners like Karpenter, no headroom is extrapolated and the check can block scale-ups the cluster would actually absorb.
