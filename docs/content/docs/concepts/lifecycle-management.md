...
- Upgrade snapshots, taken automatically whenever the upgrade mode demands one. They are required for correct operation: the operator records the resulting path and restores from it.
- Manual snapshots, triggered on demand.
- Periodic snapshots, taken on a schedule.

The manual and periodic kinds are optional extras for backup and forking: they never participate in upgrades, and automatic recovery always uses the latest checkpoint information instead.
...
|--------------------|----------------------------------------------------------------------------------------------------------------------------------------------------------------------|-------------------------------------------------------------------------------------------------------------------------------------------------------|
| The resource       | Removed, together with everything the operator materialized, the complete object tree described in the [Deployment Overview]({{< ref "docs/deployment/overview" >}}) | Stays in place, with the failure recorded in its status                                                                                               |
| The tracked state  | Removed with it: a redeployment afterwards starts from empty state unless a starting snapshot is provided explicitly                                                 | Retained, keeping recovery open through the automatic restarts under [Self-Healing and Rollback](#self-healing-and-rollback) or a manual redeployment |
| The snapshot files | Follow their owner: savepoints remain on storage unless disposal was requested, checkpoints belong to Flink and may be removed by it at any time                     | Left untouched                                                                                                                                        |
