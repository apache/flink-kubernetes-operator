...
An autoscaler that chased every fluctuation would restart the job constantly and do more harm than good, so the design leans deliberately toward calm:

- Metrics are averaged over a time window, so decisions respond to sustained trends rather than spikes.
- Around the target utilization sits a tolerance band, and as long as a vertex stays inside it, nothing happens at all.
- After every rescale a stabilization period gives the job time to settle before new measurements count.
- Capacity is released more cautiously than it is added, so a brief dip does not immediately take away headroom that the next peak will need.
