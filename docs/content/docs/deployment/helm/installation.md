...
[Apache Flink release signing keys](https://downloads.apache.org/flink/KEYS):

```
gpg --no-default-keyring --keyring flink-keys.kbx --import KEYS
gpg --no-default-keyring --keyring flink-keys.kbx --export > flink.gpg
```

Helm only reads legacy binary keyrings, whereas `gpg --import` writes a keybox on GnuPG 2.1 and later,
which is why the keys have to be exported separately with `gpg --export`.
