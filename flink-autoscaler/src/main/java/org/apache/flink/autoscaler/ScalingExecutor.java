...
public class ScalingExecutor<KEY, Context extends JobAutoScalerContext<KEY>> {
    ...
    private Map<JobVertexID, ScalingSummary> applyCustomExecutors(
        ...
        for (Map.Entry<String, ScalingExecutorPlugin<KEY>> entry : chain) {
            ...
            // Merge the job configuration with the prefix-stripped per-plugin overrides so the
            // plugin sees its effective configuration, consistent with the custom evaluator plugin.
            var mergedConfiguration = new Configuration(conf);
            mergedConfiguration.addAll(
                    AutoScalerOptions.customScalingExecutorConfiguration(conf, instanceName));
            var configuration = new UnmodifiableConfiguration(mergedConfiguration);
